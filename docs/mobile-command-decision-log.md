...
- Alternatives considered: nested Next.js application; separate top-level mobile application; separate repository/package.
- Capability-gate effect: Mobile packaging must remain additive and must not weaken desktop chat or local agent execution.
- Failure/fallback behavior: Until accepted, no production mobile package or build wiring may be created; desktop Lily remains unchanged.
- Compatibility/migration effect: Must define dependency isolation, shared schema consumption, deployment URLs, and migration from any prototype location.
- Supersedes: None.
...
- Alternatives considered: Capacitor; platform-native iOS/Android shells; PWA-only baseline; another embedded-web shell.
- Capability-gate effect: Native capability failure must preserve the web Chat Only baseline; missing authority fails safe.
- Failure/fallback behavior: Unsupported native methods return explicit errors and leave text chat available; no unconfigured native fallback.
- Compatibility/migration effect: Must freeze bridge versioning, plugin ownership, minimum OS versions, signing, and web-only degradation.
- Supersedes: None.
...
- Alternatives considered: user-primary ownership; license-primary ownership; device-primary ownership; explicit linked identity domains.
- Capability-gate effect: Identity uncertainty must reject pairing/control authority without invalidating local desktop licensing or local sessions.
- Failure/fallback behavior: Authentication/binding ambiguity denies remote actions; local Lily remains usable.
- Compatibility/migration effect: Must specify existing-device compatibility, key rotation, revocation cascades, account sign-out, license change, and desktop replacement.
- Supersedes: None.
...
- Alternatives considered: self-hosted coturn; managed TURN; hybrid regional topology; relay-only or P2P-first policy.
- Capability-gate effect: WebRTC/TURN failure must degrade to Chat Only and never terminate or weaken the local Lily session.
- Failure/fallback behavior: Bounded ICE/reconnect attempts then explicit `failed_chat_only`; no endless retry or silent control continuation.
- Compatibility/migration effect: Must cover credential/version compatibility, region rollout, provider migration, and old-client feature disablement.
- Supersedes: None.
...
- Alternatives considered: Electron-only capture; Windows Graphics Capture helper; Node native addon; Rust/C++ helper using `SendInput`; observe-only degradation.
- Capability-gate effect: Helper failure cannot affect desktop chat; input outside the approved surface must be rejected.
- Failure/fallback behavior: Input failure downgrades to observe or Chat Only as authority permits; crash fails loud and revokes control.
- Compatibility/migration effect: Must cover Windows versions, DPI/multi-monitor mapping, code signing, updater compatibility, and helper protocol versioning.
- Supersedes: None.
...
- Alternatives considered: Electron capture; ScreenCaptureKit helper; CGEvent helper; observe-only degradation.
- Capability-gate effect: Missing Screen Recording or Accessibility authority denies the affected mode while preserving Chat Only.
- Failure/fallback behavior: Permission denial or helper failure produces explicit recoverable state; control is revoked, never continued invisibly.
- Compatibility/migration effect: Must cover supported macOS versions, permission changes, hardened runtime, entitlements, notarization, and helper protocol versioning.
- Supersedes: None.
...
- Alternatives considered: Wayland portal observe only; X11 capture/control; helper-based control; Linux Chat Only.
- Capability-gate effect: Unsupported Linux environments must fail loud to Chat Only rather than advertise unsafe or nonfunctional control.
- Failure/fallback behavior: Missing portal/helper/authority disables Live capability without affecting local Lily.
- Compatibility/migration effect: Must publish a platform matrix and ensure release metadata never advertises an unverified capability.
- Supersedes: None.
...
- Alternatives considered: browser speech; native OS speech; Lily streaming ASR; Lily non-streaming ASR; configured model transcription; text-only fallback.
- Capability-gate effect: ASR failure preserves existing typed text and partial transcript; voice never bypasses approval or switches to an unconfigured provider.
- Failure/fallback behavior: Follow only the accepted configured chain, then retain the draft and return to text input.
- Compatibility/migration effect: Must define server-config compatibility, disabled-provider behavior, privacy copy, credential rotation, and older web/native clients.
- Supersedes: None.
...
- Alternatives considered: direct APNs plus FCM; managed aggregation service; no push in baseline; platform-specific staged rollout.
- Capability-gate effect: Push is advisory only; failure cannot block commands, approvals, or local Lily, and notification content cannot grant authority.
- Failure/fallback behavior: Commands remain available on reconnect/poll; invalid tokens are retired; sensitive content is omitted.
- Compatibility/migration effect: Must cover token rotation, app environment separation, old clients, provider outage, and device revocation.
- Supersedes: None.
...
- Alternatives considered: existing object storage; dedicated object bucket; server-local staging; provider-managed temporary storage.
- Capability-gate effect: Storage failure must preserve local chat and drafts; unverifiable or expired objects must not enter agent execution.
- Failure/fallback behavior: Fail upload explicitly, preserve retriable metadata where safe, and delete/expire objects according to the accepted policy.
- Compatibility/migration effect: Must define object-key/version compatibility, region migration, stale-client URLs, and deletion during provider change.
- Supersedes: None.
...
- Alternatives considered: existing Lily remote config; dedicated Mobile Command config; build-time flags; hybrid signed remote config.
- Capability-gate effect: Unknown, expired, or unavailable remote authority disables sensitive remote capabilities while preserving today's desktop/local behavior.
- Failure/fallback behavior: Fail open to the existing desktop baseline and fail safe for pairing, observe, control, upload, voice-provider, and push authority.
- Compatibility/migration effect: Must define additive fields, older-client ignore behavior, rollback, cached-config expiry, and scope changes.
- Supersedes: None.
...
- Alternatives considered: lockstep release; independently versioned components with compatibility matrix; desktop/server first then mobile; feature-flagged staged rollout.
- Capability-gate effect: Mixed versions and rollback must never remove existing desktop capability; unsupported remote features stay disabled.
- Failure/fallback behavior: Incompatible Mobile Command surfaces fail to an explicit upgrade/Chat Only state without changing local desktop sessions.
- Compatibility/migration effect: This decision owns the compatibility window, protocol/schema versions, migration sequencing, store delay, rollback, and deprecation policy.
- Supersedes: None.

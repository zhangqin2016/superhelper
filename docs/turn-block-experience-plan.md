...
1. **失败重试**：失败 turn 的卡片带"重试"按钮（retryLastMessage IPC 原本就有、
   只缺 UI）；点击时校验仍是末条消息，过期则提示并移除。
2. **按会话草稿**：composer 输入随打随存（内存 Map），切会话恢复；发送成功
   清除、发送失败恢复。
3. ~~"有新消息 ↓"悬浮条~~ **已撤销**：真机验收发现既有的
   `scrollToBottomBtn`（回到最新）已覆盖该场景，悬浮条属重复建设且
   触发过宽（任何视觉更新即出现）。教训：审计先查既有 affordance。
4. **停止状态可见**：Esc/中断按钮触发后本地乐观置 phase=stopping，状态行
   显示"正在停止…"，终态事件到达后自然覆盖。
5. **审批防双击**：权限/方案/问题卡的 action 按钮点击后整组锁定，失败解锁
   （成功后卡片自然消失）——同一 requestId 不会被响应两次。

## 交互打磨第二/三批（已完成）

6. **整条回答复制**：封存回答卡 hover 出现"复制回答"（与重试合并为动作行），
   复制完整原始文本。
7. **[审计修正]** 工具输出全文查看本就存在（appendToolResultBlock 的
   展开/收起+复制）；真实问题是 fullText 全量过 IPC——记为 M5 余项
   （主进程 LRU + 按需拉取）。
8. **@补全键盘导航**：↑↓ 循环选择、Enter/Tab 确认（弹层打开时 Enter
   不再触发发送）、高亮当前项。
9. **用户消息重新编辑**：用户消息卡 hover"重新编辑"→ 文本回填输入框
   （注：无对话回退，编辑后作为新消息发送）。
10. **会话内搜索**（⌘F/Ctrl+F）：查找条按消息卡匹配，Enter/Shift+Enter
    上下条，当前命中高亮滚动居中，Esc 关闭回焦输入框。
...
3. ✅ **workspace digest（环节 A）**：会话准备时把"工作区地图"注入
   AGENT.md——两层目录树（带子项计数，跳过 node_modules/.lily-work 等噪声）
   + 最近修改文件清单（带日期）。有界扫描（50 行/2000 项/4k 字符），
   digest 进缓存签名，工作区变化即随下次准备刷新。模型醒来即知地形，
   省掉笨模型最易出错的盲目搜索轮。
4. 待做（按性价比）：技能运行时验收+重试；网关任务路由 + 按模型
   质量度量；L1 自动提取候选记忆（等显式"记住"验证后）。

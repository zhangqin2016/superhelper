...
| Engine message/event normalization | `src/main/runtime/opencode-message-parts.js:526-535`; `src/main/runtime/opencode-runtime-reducer.js:739-842`; `src/main/opencode-agent-session.js:1303-1350`; `src/main/runtime-event-bus.js:20-73` | `event-projector.js` consumes normalized Lily events and sealed records, but planned durable projection journal/epoch/sequence owns reconnect cursors; current in-memory bus seq is source metadata only |
...
`remote-api.js` is a facade over narrow `service-client.js` exports and owns no fetch/sign/key/region/retry logic. Per [MC-SPEC-008](mobile-command-agent-bridge-contract.md), planned `TurnOrchestrator.admitExternalCommand` co-persists exactly-once admission with the one queue/history. Dispatch separately durably CASes `admitted→dispatching` before at most one engine call, then records `engine_accepted`; recovery of `dispatching` becomes `dispatch_unknown` and never auto-replays. Current remote steer writes one queue item and never calls `runner.steer`. The per-session admission lock also arbitrates desktop/remote durable FIFO and ownership-safe revoke/cancel. `event-projector.js` must add a durable per-session journal/state seam with `projectionEpoch`, monotonic `projectionSeq`, atomic append+high-water-before-publish, epoch rotation, and snapshot-under-lock; current `RuntimeEventBus.seq` is never a durable cursor. Admission/queue/snapshot projections preserve exact mode/downgrade names. `artifact-source.js` and `upload-staging-adapter.js` remain narrow adapters. OS adapter filenames remain evidence-needed under MC-ADR-005–007.
...
| `src/main/turn-orchestrator.js` | Add planned admission lock/seam, ledger states `admitted/dispatching/engine_accepted/dispatch_unknown/terminal`, pre-engine durable CAS, at-most-one automatic attempt, durable commit-order FIFO, and ownership-safe revoke/cancel; never claim exactly-once execution |
...
Tests remain planned under auto-discovered `scripts/test-*.mjs`. Minimum boundaries include exactly-once admission versus at-most-one dispatch attempt, every dispatch crash window, no automatic replay, future engine lookup, durable desktop/remote FIFO arbitration, ownership-safe revoke, remote steer normalization, durable projection journal/epoch/high-water and snapshot cut/gap recovery, permission/question authority, artifact/upload safety, lifecycle disablement, config kill switches, and capability-gate fallback.

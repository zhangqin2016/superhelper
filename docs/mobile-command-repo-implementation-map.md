...
`remote-api.js` is a facade over narrow `service-client.js` exports and owns no fetch/sign/key/region/retry logic. Per [MC-SPEC-008](mobile-command-agent-bridge-contract.md), planned `TurnOrchestrator.admitExternalCommand` co-persists exactly-once admission on a durable FIFO/idle-turn input record without claiming a started turn. Dispatch separately CASes `admitted→dispatching` before at most one engine call; `dispatching` recovery becomes `dispatch_unknown`, so even normalized steer may have zero confirmed turn. The session lock arbitrates FIFO and ownership-safe revoke/cancel. `agent-bridge.js` orchestrates authorized snapshot request/assembly. `event-projector.js` owns durable `projectionEpoch`/`projectionSeq` journal, append+high-water-before-publish, epoch rotation, and the atomic snapshot cursor cut; current `RuntimeEventBus.seq` is never a durable cursor. Admission/queue/snapshot projections preserve exact mode/downgrade names. Other adapters remain narrow. OS adapter filenames remain evidence-needed under MC-ADR-005–007.

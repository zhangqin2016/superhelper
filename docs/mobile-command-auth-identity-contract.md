...
| Remote access token | Lily server / signaling and command APIs for exactly one remote session | signed/opaque bearer with `typ=remote_access`, remote session, full subject tuple, token generation, scopes, `iat`,`exp`,`jti` | 5 min | mobile memory and desktop process memory; never disk | minted from one-time remote refresh token | TLS, expiry, audience/scope, subject tuple, session state, device signature on mutations; `jti` denylist after incident | revoke session/token family on logout/device/grant/license/key anomaly; stolen token without mobile key cannot mutate and is denied after revoke; no offline authority |
...
1. The mobile and desktop must both have active `user_devices` rows for the same `user_id`.
2. Their `mobile_device_roles` must be distinct and correct.
3. The selected `license_id` must be active, unexpired, and actively bound to the target desktop through `license_devices`. A mobile license binding cannot substitute for the desktop binding.
4. `account_session_id` must belong to the same `user_id` and mobile `device_id`, be unexpired and unrevoked.
5. The paired grant must match the entire tuple and be active.
...
7. Server checks route/routing authority; desktop repeats grant, signature, session, permission, approval, and expiry checks before local effects.
...
sequenceDiagram
  ...
  S->>DB: Find current user_session by old token hash
  alt current hash found
    S->>DB: BEGIN; lock current user_session row

...
| B | Task 13 好友和消息交互 | 好友操作、持久社交命令、持久编辑/撤回、持久已读/精确计数、引用回复和键盘@选择已独立双审；编辑撤回操作UI、完整收件箱与可见已读仍待接通并验证 |
...
键盘@选择切片：保持纯textarea和既有`mentionUserIds`，不引入富文本或名字→身份猜测。在现有composer中用“提醒成员”可移除标签明确展示提醒对象；输入空白边界的`@查询`或点击行内提醒入口打开授权候选。选择只消费仍匹配的当前`@查询`片段并加入稳定ID标签，不在正文里伪造有身份绑定的`@名字`。标签与正文独立，文案明确只有所选成员收到本次@提醒，正文中的@文字不会自动加入提醒对象；手动正文编辑不暗中改变提醒对象。

候选复用已完成的主进程只读接口，complete/unknown/error明确区分，按当前会话授权获取，不从好友/Team目录补名册。按名字、Lily ID和用户ID筛选，重复名字显示身份区分；1000项完整可达，不静默截断。方向键/Enter选择，Escape或Tab关闭，IME期间不选人/发消息，候选打开时Enter不得误发送。未确认候选时已有草稿ID保留并显示可移除的未知标签，明确complete名单缺席的ID标为不可用，不自动清稿。所有名称为安全文本，标签只持久化排序去重的ID；失败同意图保持原UUID，新增/移除提醒成员形成新意图，旧ACK不得清除更新后的标签。候选请求和旧按钮按会话、账号、显隐、请求代次围栏；同步/授权变化重新获取候选，普通输入不能饿死正在完成的请求。接收消息的提醒标记只读显式IDs，自己的身份来自当前安全目录，不解析正文HTML。三语言、RTL及实际Electron键盘/迟到响应/草稿/发送测试先RED后实现；与引用交互回归共同验证，尚不关闭编辑撤回、收件箱和可见已读项。
...
2026-08-31 键盘@选择完成规格→质量独立审查及复审。候选、标签、整份草稿/命令意图、授权刷新、时间线显式提醒和三语言RTL已接通；1000候选全部可达，1000标签限制在可滚动区域。独立导航覆盖迟到候选、旧按钮、跨会话发送ACK、未知/失败、账号重置与撤权再授予；真实签名PG验证正文@/HTML不产生提醒、显式候选ID只增加一次提醒，生产bootstrap授权解密并写入SQLite后才能ACK。

SPEC实际复现并修复原生按钮输入法误激活：仅return没有阻止浏览器默认click，isComposing/229与Enter/空格共16种按钮组合会误选、移除或开关列表。现只在候选控件root阻止默认动作，正文textarea的输入法处理不变；16种先RED后GREEN，原独立reproducer两种标记均无写入/发送且候选保持打开。测试使用真实Electron键盘默认行为并注入输入法标记，不冒充完整OS输入法或真实双客户端验收。

功能修复后父进程96项协作Node/architecture/registry、14组隔离PG集成全部通过，完整能力门禁185项退出0。门禁仍有bundled OpenCode shape/usage缺失跳过和121行既有Renderer测试宿主IPC诊断；不表示全项目零跳过、私有bucket或生产验收。质量审查无阻断问题，提示文案明确限定为@提醒而非普通消息通知。本检查点只关闭键盘@切片，Task13其他交互和后续里程碑未完成；未合并或部署。

最后三语言提示文案和首句回归守卫随后完成独立复核；父进程再次运行实际Electron mentions-ui（含小窗/RTL/16种输入法标记组合）、locale和registry守卫，全部通过。上述185项完整门禁执行于这一纯文案调整前，不能记成文案调整后的全量重跑。

### 当前依赖：消息操作恢复读模型（已完成，不能代替操作 UI）

现有 `getState.outbox` 只有无正文的排队元数据，无法区分创建/编辑/撤回，也不保留失败原因。保持这个广播轻量，不在每次状态事件中解密全部历史 outbox。新增窄的 `readMessageOperations({conversationId,outboxIds})`：每批最多200个严格且不重复的ID，只读取当前账号、仍授权会话内的指定记录，返回绑定会话的安全类型化视图及明确缺失ID。白名单包含命令类型、目标消息、预期revision、原UUID、状态、attempts、确认/不确定标记、同会话阻塞ID、是否要求原设备；只有编辑操作附带用户自己已加密保存的编辑正文，不暴露originDeviceId、附件路径、信封、凭据或任意错误文本。缺失与错会话不得混成成功记录，未知命令不得猜成创建；已停止/切换账号和撤权后不能返回旧视图。

失败原因用加性本地迁移保存有限错误码，不保存服务端错误message/stack。错误码与对应状态CAS或重试计数同一写入，成功结算清除；旧行明确无原因，损坏/未知码只给通用失败。覆盖revision冲突、编辑/撤回过期、明确拒绝、可重试网络和响应不确定；失败原因不能覆盖deliveryUncertain的回执优先语义。补闭合的skip IPC/preload，复用原outbox状态机与原设备限制，绝不把已确认/可能已提交操作本地取消。确认跳过/取消后通知界面并继续已有排队命令，迟到drain仍只允许queued状态；继续重试不换UUID、不改变正文或revision。

该切片先用真实SQLite/重启、实际IPC/preload和生产outbox验证读模型、字段白名单、200项边界、跨会话/账号/撤权、错误码落盘及CAS、同会话屏障、原设备和迟到结果；保留已有测试意图，不降低门禁。之后仍须独立实现未提交编辑稿的加密持久化、行内编辑/撤回确认/冲突比较与恢复按钮，以及Electron交互验收。

2026-09-01 此依赖切片完成规格与质量独立审查。SQLite v15 已持久化有限错误码；窄读模型、IPC/preload、原设备 skip/cancel、队列继续及回执优先语义均已接通。真实签名 PostgreSQL 联调证明：丢失编辑 ACK 后可由回执清除不确定原因；并发形成的 revision 冲突会保留原加密编辑意图，重启后仍可安全读取；只有明确跳过确定未提交的失败编辑，后续排队创建才发送，服务端没有拒绝编辑事件或重复消息。

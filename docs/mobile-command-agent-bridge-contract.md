...
- `RuntimeEventBus` and the orchestrator's normalized runtime events are projection inputs, but the current bus sequence is in-memory and is not a reconnect cursor. The planned durable projection journal below owns remote cursors. Raw engine events are prohibited.
...
2. Mobile Command never creates, resumes, mirrors, or writes a hidden second history. The local Lily transcript, turn archive, and queue remain the only conversation truth; the projection journal is a derived delivery log, not a second history.
...
|---|---|---|
| idle | `queue` or `steer` | Atomically admit through `admitExternalCommand` by attaching ownership metadata to one durable FIFO/idle-turn input record. Execution then follows the dispatch state machine; admission does not claim a started or accepted engine turn. |
...
### 3.3 Exactly-Once Admission, At-Most-One Automatic Dispatch Attempt
...
3. Create the ledger record in `admitted` while atomically attaching the same command metadata to exactly one FIFO item or idle-turn input record. With today's engine, requested steer always admits as queue with `effectiveMode=queue` and never invokes `runner.steer`.
4. Flush ledger and queue/turn-input/history mutation together. Only this flush establishes exactly-once **admission**. It does not prove engine execution or exactly-once side effects.

Required ledger fields are: `schemaVersion`, `lilySessionId`, `commandId`, `idempotencyKey`, canonical `payloadHash`, `desktopDeviceId`, `mobileDeviceId`, `remoteSessionId`, signed source sequence, `requestedMode`, `effectiveMode`, nullable `downgradeReason`, state, `dispatchAttemptedAt`, `engineAcceptedAt`, `queueItemId` or `turnId`/committed-user reference, ownership/source, `createdAt`, `updatedAt`, terminal type/error reference, and `retainUntil`. The minimum execution states are `admitted`, `dispatching`, `engine_accepted`, `dispatch_unknown`, and `terminal`; terminal detail separately records `completed`, `failed`, `interrupted`, or `stalled`. Payload conflict is a rejected admission response, not an admitted execution state.

Every successful admission response MUST forward the persisted `commandId`, `idempotencyKey`, state and destination reference, and MUST forward/transform the ledger fields under these exact names: `requestedMode`, `effectiveMode`, and nullable `downgradeReason`. A current requested steer response therefore reports `{ requestedMode: "steer", effectiveMode: "queue", downgradeReason: "STEER_IDEMPOTENCY_UNAVAILABLE" }`; a normal queue reports `{ requestedMode: "queue", effectiveMode: "queue", downgradeReason: null }`. The mobile client MUST render these as distinct outcomes and MUST NOT label a downgraded steer as either a successful steer or an ordinary user-requested queue. `admitted` means durable local admission only; it MUST NOT be labeled executed.

Dispatch is a separate state machine under the same per-session lock:

1. Select the FIFO head in `admitted`, durably compare-and-set it to `dispatching`, set `dispatchAttemptedAt`, and flush **before** calling the engine.
2. After that flush, the dispatcher may call the engine at most once for this command. No retry/admission/recovery path may make another automatic call.
3. If the engine confirms acceptance, durably transition to `engine_accepted` and attach the engine/turn reference. A Lily terminal event transitions to `terminal` with its terminal detail.
4. If the process crashes, disconnects, or loses the response while state is `dispatching`, recovery MUST NOT resend. It transitions to `dispatch_unknown` (or terminal `DELIVERY_UNKNOWN`) and projects that the command may or may not have executed. Mobile and desktop must show this ambiguity. The user may explicitly redo the intent only with a new `commandId`/idempotency key; same-key retry only returns `dispatch_unknown`.
5. This guarantees exactly-once admission and at-most-one automatic engine-dispatch attempt, **not** exactly-once execution or side effects. Only a future engine that idempotently accepts `commandId` and supports durable outcome lookup may automatically reconcile `dispatching` to accepted/not-accepted and safely dispatch when proven not accepted.

Crash semantics are normative:

- Crash before admission commit leaves neither ledger admission nor queue/turn/history mutation; same-key retry is safe.
- Crash after admission commit but before durable CAS to `dispatching` recovers `admitted`; the normal FIFO dispatcher may make its one automatic attempt.
- Crash after durable CAS to `dispatching`, whether before, during, or after the engine call, recovers as `dispatch_unknown` and never automatically resends because current engine outcome is unknowable.
- If a caller cannot determine whether admission commit completed, it returns `DELIVERY_UNKNOWN`. Startup/reconnect MUST first recover and reconcile the ledger, queue, active turn, and committed history from that same session store. It MUST NOT call the engine while admission is indeterminate.
...
Terminalization updates the ledger in the same durable turn-finalization mutation that seals the local terminal record. `dispatch_unknown` and terminal ledger records remain at least through the remote-session lifetime, event replay window, and reconnect/offline retry window. Cleanup is allowed only after `retainUntil`, no queue/active-turn/history reference remains unresolved, and the terminal/archive-or-unknown projection is durable. Expiry never authorizes blind replay: an absent/ambiguous old key requires an explicit user-confirmed resend with a new key.
...
- Ordinary desktop messages and validated remote commands acquire the same per-session admission lock. Their durable commit order defines FIFO; an ordinary desktop message does not preempt a remote item that already committed. The arbitration point is the atomic queue/admission mutation, not UI arrival time or relay timestamp.
- Desktop authority actions—device revoke, ownership-safe cancel, or explicit interrupt—use that same lock and may atomically remove/terminalize eligible owned remote items before the next dispatch. They never cancel desktop or other-device items by broad queue clearing.
- Revoking a mobile device transactionally finds every queue command owned by that device in `admitted` (not yet `dispatching`), removes it from FIFO, terminalizes it as revoked/cancelled, and writes audit plus projection in the same mutation. It immediately blocks new commands, cancellation, answers, approvals, reconnect, and artifact access. Commands already `dispatching` or `engine_accepted` continue under the existing local-turn rule because execution may have begun, but the revoked device gains no new authority, approval/question response, artifact lookup/download, or cancellation. Desktop and other-device queue items are untouched.

## 4. Projection Source And Common Envelope

The projector consumes only normalized Lily runtime events and sealed local records. The current `RuntimeEventBus.seq` is process-memory state and MUST NOT be exposed as a durable reconnect cursor. Production requires a planned per-session projection journal/state store with `projectionEpoch`, monotonic `projectionSeq`, and durable high-water mark.

For every projected change, `event-projector.js` must redact/transform it, append the projection journal entry, advance the high-water mark, and flush both atomically **before** publish. The remote envelope carries `lilySessionId`, `turnId` (nullable only where Lily permits it), stable projected `eventId`, `projectionEpoch`, `projectionSeq`, timestamp, projected type, schema version, and redaction/version marker. Runtime bus IDs/sequence may be stored only as source correlation metadata.

Restart recovery loads the durable epoch/high-water. It must never reuse an in-memory cursor as though durable. If journal continuity cannot be proven, rotate `projectionEpoch` and require a fresh snapshot; sequence values from different epochs are incomparable.
...
|---|---|---|---|
| `user.committed`, `turn.started`, `turn.accepted`, `turn.steered`, `queue.updated` | non-terminal command/queue lifecycle | IDs, ledger execution state, status, position and `requestedMode`/`effectiveMode` forwarded; nullable `downgradeReason` forwarded or transformed to an allowlisted code; raw local file paths redacted | Project `admitted`, `dispatching`, `engine_accepted`, `dispatch_unknown`, and terminal detail honestly. `queue.updated` MUST include the three mode fields for every external item using the exact ledger names. `turn.started`, `turn.accepted`, and future-gated `turn.steered` preserve them. Mobile distinguishes downgraded steer, normal queue, and unknown dispatch. |
...
Reconnect requests an authorized snapshot for each subscribed `lilySessionId` with the client's last acknowledged `projectionEpoch`/`projectionSeq`. Under the same per-session projection/admission lock, desktop recovery reconciles the command ledger with queue/active-turn/committed-history/terminal references and captures a snapshot plus the journal's current `projectionEpoch` and `snapshotSeq` high-water. The snapshot contains conversation identity/title metadata, current phase and active `turnId`, queue items visible to that device, recovered command admission/dispatch states, safe active-turn state, pending requests with freshly computed eligibility, assembled safe assistant text, safe tool/progress states, last terminal record, authorized artifact descriptors, and the cursor cut. Every external command/queue entry includes the exact same `requestedMode`, `effectiveMode`, and nullable `downgradeReason` values as its ledger and admission response.

After releasing the snapshot lock, subscription publishes only journal entries in the same `projectionEpoch` with `projectionSeq > snapshotSeq`. The client atomically applies the snapshot, then those entries. Duplicate event/sequence is ignored. Any epoch mismatch, sequence gap/out-of-order entry, unknown mandatory event, or cursor older than journal retention forces a new snapshot. The client never guesses a transition, and current bus `recent`/`seq` cannot satisfy this contract.
...
| Crash/lost response after durable `dispatching` | `dispatch_unknown` / `DELIVERY_UNKNOWN`; state that execution may or may not have occurred | Never automatically resend; at-most-one dispatch attempt | User may explicitly redo only with a new command ID/key; future engine lookup may reconcile. |
| Projection restart, cursor gap, or epoch mismatch | discard incremental assumption and request snapshot at a new/proven epoch cut | Conversation/ledger remain intact; no in-memory bus cursor is trusted | No remote mutation is authorized from stale projection state. |
| Remote `mode=steer` on current engine | exactly-once admission as durable FIFO queue; expose `requestedMode=steer`, `effectiveMode=queue`, `downgradeReason=STEER_IDEMPOTENCY_UNAVAILABLE` | Active turn is untouched; there is at most one automatic dispatch attempt, which may become `dispatch_unknown` with zero confirmed turn | Never call `runner.steer`; mobile sees admitted/dispatch/unknown state without a promise of execution. |
...
| Device revoked with queued and active work | atomically remove/terminalize only that device's `admitted` queue items and project/audit cancellation | `dispatching`/`engine_accepted` may continue; desktop and other-device items are untouched | Immediately deny all new authority, approvals/questions, reconnect, cancellation, and artifact access. |
...
- Planned desktop `agent-bridge.js` owns validation and calls the planned public `TurnOrchestrator.admitExternalCommand` seam; it does not own a separate dedup ledger. The orchestrator/session store owns atomic admission, while the bridge owns correlation, ownership-aware cancellation, and orchestration of authorized snapshot requests/assembly from allowed session state and projector output.
- Planned `event-projector.js` owns allowlisting/redaction plus the durable per-session projection journal (`projectionEpoch`, monotonic `projectionSeq`, atomic high-water-before-publish), the atomic snapshot cursor cut under the shared lock, and terminal/artifact projection. It supplies the authorized bridge assembly with the cut and journal entries; it never parses raw OpenCode events or exposes current `RuntimeEventBus.seq` as a reconnect cursor.
...
Implementation must add automated coverage for exactly-once admission separately from execution; durable CAS `admitted→dispatching` before the sole engine call; successful `engine_accepted`; crash at every pre/post-CAS and engine-call boundary producing no automatic replay and `dispatch_unknown`; explicit new-command redo; future command-ID outcome lookup; idempotency conflict; target absence; durable commit-order FIFO for concurrent desktop/remote admission; desktop ordinary-message non-preemption; authority cancel/revoke atomic priority; revoke removing only the device's `admitted` items while active work continues without new authority; cross-session concurrency; current remote steer never calling `runner.steer`, exactly-once admission, at-most-one automatic attempt, and possible `dispatch_unknown`/zero confirmed turn; persisted mode/downgrade fields; projection journal append+high-water atomicity; restart recovery/epoch rotation; bridge-authorized snapshot assembly with projector-owned atomic cursor cut; subscribe-after-cut; gap/epoch mismatch snapshot; proof current bus seq is never used as durable cursor; reasoning/secret redaction; tool and approval lifecycle; terminal/artifact failures; version skew; relay loss; and the capability-gate proof that every bridge failure preserves local Lily chat, tools, history, model, and artifacts.

...
- Planned `event-projector.js` owns allowlisting/redaction plus a durable per-session projection journal (`projectionEpoch`, monotonic `projectionSeq`, atomic high-water-before-publish), snapshot cuts, and terminal/artifact projection from normalized Lily state. It never parses raw OpenCode events or exposes current `RuntimeEventBus.seq` as a reconnect cursor.

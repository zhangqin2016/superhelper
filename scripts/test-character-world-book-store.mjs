...
try {
  ...
  check("migration v8 backfills pre-existing rows before the dedup index", () => {
    assert.equal(legacyStore.db.pragma("user_version"), 8);
  ...
  check("a v8 database opened by a v7 migration set is a designed no-op", () => {
    const pinDbPath = path.join(tmp, "forward-pin.db");
    const pinSeed = openDatabase(pinDbPath);
    pinSeed.migrate(MIGRATIONS);
    assert.equal(pinSeed.pragma("user_version"), 8);
    pinSeed.close();
    const pinDb = openDatabase(pinDbPath);
    assert.equal(pinDb.migrate(MIGRATIONS.slice(0, 7)), 8);
    assert.equal(pinDb.pragma("user_version"), 8);
    pinDb.close();
  });

...
try {
  ...
  check("migration v8 backfills pre-existing rows before the dedup index", () => {
    assert.equal(legacyStore.db.pragma("user_version"), 9);
  ...
  check("a v9 database opened by a v7 migration set is a designed no-op", () => {
    const pinDbPath = path.join(tmp, "forward-pin.db");
    const pinSeed = openDatabase(pinDbPath);
    pinSeed.migrate(MIGRATIONS);
    assert.equal(pinSeed.pragma("user_version"), 9);
    pinSeed.close();
    const pinDb = openDatabase(pinDbPath);
    assert.equal(pinDb.migrate(MIGRATIONS.slice(0, 7)), 9);
    assert.equal(pinDb.pragma("user_version"), 9);
    pinDb.close();
  });

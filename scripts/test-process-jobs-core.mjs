...
try {
  const job = await startJob({
    ...
    args: ["-e", "process.on('SIGTERM',()=>{}); console.log('ready'); console.log('[lily-progress] {\"label\":\"index\",\"current\":2,\"total\":5,\"domain\":\"file-index\"}'); setInterval(() => console.log('tick'), 1000);"],
    cwd: tmp,
  ...
  const logs = logsJob({ jobId: job.jobId, tailBytes: 10_000 }, { registryDir: tmp, writerLockPath:path.join(fs.realpathSync(tmp), "writer.sqlite") });
  ...
  const stopped = await stopJob({ jobId: job.jobId, timeoutMs: 200 }, { registryDir: tmp, writerLockPath:path.join(fs.realpathSync(tmp), "writer.sqlite") });
  assert(stopped.ok === true && stopped.stopped === true, `job_stop should stop the process: ${JSON.stringify(stopped)}`);

  require("../src/main/collaboration/foreground-writer").createForegroundWriter({filePath:path.join(fs.realpathSync(tmp),"writer.sqlite")}).run(()=>{});

  const afterStop = await statusJob({ jobId: job.jobId }, { registryDir: tmp, writerLockPath:path.join(fs.realpathSync(tmp), "writer.sqlite") });
  ...
  const mediaLogs = logsJob({ jobId: mediaJob.jobId, tailBytes: 10_000 }, { registryDir: tmp, writerLockPath:path.join(fs.realpathSync(tmp), "writer.sqlite") });
  ...
  const mediaStatus = await statusJob({ jobId: mediaJob.jobId }, { registryDir: tmp, writerLockPath:path.join(fs.realpathSync(tmp), "writer.sqlite") });

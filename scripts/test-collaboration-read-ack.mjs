...
try {
  ...
  assert.equal((await service.markRead({ conversationId: "c", seq: 2 })).seq, 100,
    "an already-higher server read watermark remains authoritative");
  response = { ok: true, result: { lastReadSeq: 0 } };
  assert.equal((await service.markRead({ conversationId: "c", seq: 99 })).seq, 0);
  ...
  for (const malformed of [{}, { lastReadSeq: 99 }, { ok: false, result: { lastReadSeq: 99 } }, ...[null, "5", -1, 1.5, Number.MAX_SAFE_INTEGER + 1].map((lastReadSeq) => ({ ok: true, result: { lastReadSeq } }))]) {
    response = malformed;
    assert.deepEqual(await service.markRead({ conversationId: "c", seq: 99 }), { ok: false, code: "COLLAB_READ_ACK_INVALID" },

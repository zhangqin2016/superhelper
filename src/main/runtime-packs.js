...
// userData resolution lives in config now (honours LILY_USER_DATA_DIR for agent
// subprocesses / CLIs, falls back to the host's injected path otherwise), so the
// CLI and the main process always resolve the SAME location with no special case.
function packsRoot() {
...
async function defaultResolveArtifact(id, plat) {
  ...
  const res = await require("./service-client").runtimePackArtifact(id, plat);

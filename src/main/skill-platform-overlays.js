...
const SKILL_PLATFORM_OVERLAYS = Object.freeze({
  "anthropics-docx": Object.freeze({
    en: "anthropics-docx: Lily's bundled python-docx is the canonical scratch-authoring route. The absence of the Node docx package does not mean Word generation is unavailable. Preserve supplied templates when present, then render and inspect the output before claiming delivery verification.",
    zh: "anthropics-docx：Lily 内置的 python-docx 是从零生成 Word 的标准路线；缺少 Node docx 包不代表平台不能生成 Word。有模板时优先保真编辑模板，交付前必须渲染并实际检查成品。",
  }),
  "anthropics-pptx": Object.freeze({
    en: "anthropics-pptx: Lily's bundled python-pptx is the canonical scratch-authoring route; missing pptxgenjs is not missing presentation capability. Subagent QA is use-if-available. Otherwise run the same visual QA inline and never skip it.",
    zh: "anthropics-pptx：Lily 内置的 python-pptx 是从零生成演示文稿的标准路线；缺少 pptxgenjs 不代表平台没有 PPT 能力。子代理 QA 可用则用，否则在当前会话内联执行同样的视觉检查，绝不能跳过。",
  }),

...
const PROCESS_JOB_PROTOCOL_GUIDANCE = [
  ...
  "Use job_start with an explicit cwd and, when possible, a healthcheck (process, tcp, http, or log). Then use job_status and job_logs to verify startup before claiming success. Use job_stop when the user asks to stop a managed job. Automatic follow-up is opt-in: use replayPolicy=inspect for post-exit inspection or replayPolicy=idempotent only when rerunning cannot duplicate side effects; the default replayPolicy=never never starts a hidden follow-up turn.",

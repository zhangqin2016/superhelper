...
function removeScopeRows(store, scopeId) {
  ...
  const ids = store.db.all(`SELECT id AS conversation_id FROM conversations WHERE account_id = ? AND scope_id = ?
    ...
    UNION SELECT conversation_id FROM edit_drafts WHERE account_id = ? AND scope_id = ?
    UNION SELECT conversation_id FROM read_checkpoints WHERE account_id = ? AND scope_id = ?
    UNION SELECT conversation_id FROM task_workspace_records WHERE account_id = ? AND scope_id = ?`,
  store.accountId, scopeId, store.accountId, scopeId, store.accountId, scopeId, store.accountId, scopeId, store.accountId, scopeId, store.accountId, scopeId, store.accountId, scopeId);
  let deletedOutbox = 0;
...
function prepareBootstrapAccess(store, conversations, teams, scopeFor) {
  ...
  if (Array.isArray(teams)) {
    ...
    const oldScopes = store.db.all(`SELECT scope_id FROM conversations WHERE account_id = ? UNION SELECT scope_id FROM outbox WHERE account_id = ?
      UNION SELECT scope_id FROM drafts WHERE account_id = ? UNION SELECT scope_id FROM edit_drafts WHERE account_id = ? UNION SELECT scope_id FROM transfers WHERE account_id = ? UNION SELECT scope_id FROM share_mappings WHERE account_id = ?
      ...
      UNION SELECT scope_id FROM social_commands WHERE account_id = ? UNION SELECT scope_id FROM task_commands WHERE account_id = ?
      UNION SELECT scope_id FROM task_workspace_records WHERE account_id = ?`,
    store.accountId, store.accountId, store.accountId, store.accountId, store.accountId, store.accountId, store.accountId, store.accountId, store.accountId, store.accountId, store.accountId);
  ...
  for (const row of store.db.all(`SELECT id, scope_id FROM conversations WHERE account_id = ? UNION SELECT conversation_id AS id, scope_id FROM read_checkpoints WHERE account_id = ?
    UNION SELECT conversation_id AS id, scope_id FROM task_workspace_records WHERE account_id = ?`, store.accountId, store.accountId, store.accountId)) {

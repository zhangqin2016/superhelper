...
class TaskGit {
  ...
  async ensureWorktree({baseline,workRoot,manifest,materializePaths=null}) {
    ...
    fs.writeFileSync(path.join(target,"lily-task.json"),JSON.stringify({commit:baseline.commit,ref:baseline.ref,workRoot,dev:stat.dev,ino:stat.ino,fileIdentities,materializedPaths}),{flag:"wx",mode:0o600});
  ...
  async materializePaths({baseline,workRoot,manifest,paths}) {
    ...
    fs.writeFileSync(temporary,JSON.stringify({...saved,fileIdentities,materializedPaths}),{flag:"wx",mode:0o600});

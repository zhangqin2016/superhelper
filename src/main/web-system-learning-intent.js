...
function buildWebSystemLearningPrompt(userText) {
  ...
  return [
    ...
    "7. 扫描完成后必须运行 `scripts/finalize_web_system_learning.cjs --scan <scan.json> --contracts <api-contracts.json> --system-id <id> --name <name>`；它会确定性生成 `web-system-spec.json` 并调用 `create_web_system_skill.cjs` 生成当前工作区的 learned skill。",
    "8. 扫描必须以前台 Bash/tool 命令执行并等待完成；禁止使用 `&`、`nohup`、`setsid`、`disown` 或另开后台进程。",
    "9. 只有真实工具还在运行时才可以说“扫描正在运行/等待完成”；如果没有前台工具在跑，必须先执行扫描或说明缺失条件。",
    "10. 不允许以“我将继续扫描/下一步采集/等待分析”作为最终答复。最终答复必须来自 finalizer 的结果：要么 `ok:true` 并说明技能草稿目录、页面/API/能力数量和覆盖缺口；要么给出明确错误和可恢复动作。",
    "11. 输出清晰的学习结果：已识别页面、可支持的自然语言操作、必须二次确认的高风险动作、还需要用户补充的信息。",
    "",

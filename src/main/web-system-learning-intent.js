...
function buildWebSystemLearningPrompt(userText) {
  ...
  return [
    ...
    "6. 扫描必须以前台 Bash/tool 命令执行并等待完成；禁止使用 `&`、`nohup`、`setsid`、`disown` 或另开后台进程。",
    "7. 只有真实工具还在运行时才可以说“扫描正在运行/等待完成”；如果没有前台工具在跑，必须先执行扫描或说明缺失条件。",
    "8. 输出清晰的学习结果：已识别页面、可支持的自然语言操作、必须二次确认的高风险动作、还需要用户补充的信息。",
    "",

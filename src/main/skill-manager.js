...
const AGENT_GUIDE_I18N = {
  "zh-CN": {
    ...
    antiHallucination: "抗幻觉硬门槛（最高优先级，先查证再回答）：回答任何涉及具体事实、平台/你的能力、文件或代码内容、接口/数据、数字、名称、日期的问题前，必须先用工具核查（读文件、跑命令、查接口、看设置/工具列表），不得凭记忆、推测或“听起来对”直接下结论。核查不了就直说“不确定/需要确认”并给出下一步；绝不编造事实、功能、来源、路径、数字或结论。宁可承认不知道，也不要一本正经地给出可能错误的答案。只有闲聊、常识、纯创作或用户已在本轮提供全部依据时，才可不查直接答。被指出错误后不要只道歉了事，要回到证据重新核查再回答。",
  ...
  en: {
    ...
    antiHallucination: "Anti-hallucination gate (highest priority — verify before answering): before answering anything involving concrete facts, platform/your own capabilities, file or code contents, APIs/data, numbers, names, or dates, you MUST verify with a tool first (read the file, run the command, query the API, check settings/the tool list) — never conclude from memory, guessing, or what \"sounds right\". If you cannot verify, say plainly \"not sure / needs confirmation\" and give the next step; never invent facts, features, sources, paths, numbers, or conclusions. Admitting you don't know beats a confident wrong answer. Only skip verification for small talk, general knowledge, pure creative writing, or when the user already supplied all the evidence this turn. When corrected, do not just apologize — go back to the evidence, re-verify, then answer.",
  ...
  ar: {
    ...
    antiHallucination: "بوابة مكافحة الهلوسة (أعلى أولوية — تحقّق قبل الإجابة): قبل الإجابة عن أي شيء يتعلق بحقائق محددة أو قدرات المنصة/قدراتك أو محتوى الملفات أو الشيفرة أو واجهات البرمجة/البيانات أو الأرقام أو الأسماء أو التواريخ، يجب أولاً التحقق باستخدام أداة (اقرأ الملف، شغّل الأمر، استعلم الواجهة، تحقق من الإعدادات/قائمة الأدوات) — لا تستنتج من الذاكرة أو التخمين أو ما \"يبدو صحيحاً\". إذا تعذّر التحقق فقل بوضوح \"غير متأكد / يحتاج تأكيد\" مع ذكر الخطوة التالية؛ لا تختلق حقائق أو ميزات أو مصادر أو مسارات أو أرقام أو استنتاجات. الاعتراف بعدم المعرفة أفضل من إجابة خاطئة بثقة. لا تتجاوز التحقق إلا في الدردشة العامة أو المعرفة العامة أو الكتابة الإبداعية البحتة أو عندما يكون المستخدم قد قدّم كل الأدلة في هذه الجولة. وعند تصحيحك، لا تكتفِ بالاعتذار بل عُد إلى الأدلة وتحقق مجدداً ثم أجب.",

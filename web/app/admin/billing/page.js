...
import { upsertBillingProductAction, upsertPricingRuleAction } from "./actions";
...
export default async function AdminBillingPage() {
  const [{ products }, { rules }] = await Promise.all([
    safeApiGet("/api/admin/billing/products", { products: [] }),
    safeApiGet("/api/admin/billing/pricing-rules", { rules: [] }),
  ]);

  return (
    <AdminShell title="计费" subtitle="管理商品档位、图片/视频计价、免费次数和限额。">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody>
                ...
                {!products.length ? (
                  <tr><td className="py-6 text-slate-500" colSpan={6}>暂无商品。后续可在此创建日卡、周卡、月卡、Token 包、图片包和视频包。</td></tr>
                ) : null}
              </tbody>
            </table>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody>
                ...
                {!rules.length ? (
                  <tr><td className="py-6 text-slate-500" colSpan={6}>暂无计价规则。后续可配置图片生成和视频生成的单次价格、免费次数、每日上限和并发上限。</td></tr>
                ) : null}
              </tbody>
            </table>
          </div>

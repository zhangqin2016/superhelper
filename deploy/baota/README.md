...
`.env.example` 中协作及分享开关默认关闭。API 通过现有 `env_file: .env`
读取独立 `COLLAB_QINIU_*` 和 `COLLAB_OBJECT_KEK*`；所有部署拓扑均沿用此入口，
无需把密钥复制到 Compose、web 环境变量或客户端配置。

- `COLLAB_QINIU_BUCKET`、私有下载域名和访问凭证必须与公开发行物存储隔离。
  `COLLAB_QINIU_UPLOAD_URL` 选择 bucket 所在区域的 HTTPS 上传入口。
- `COLLAB_QINIU_PRIVATE_BUCKET=true` 只是运维确认，不会自动改变或验证七牛 ACL。
  启用前须在私有测试桶证明无签名读取被拒绝、短时签名可读取，且对象为密文。
- 对象 KEK 是独立的 32 字节随机密钥（64 位十六进制或规范 base64）。
  `COLLAB_OBJECT_KEKS` 可配置版本到密钥的 JSON 映射；当前版本由
  `COLLAB_OBJECT_KEK_VERSION` 指定。保留旧版本以读取存量对象，不得提前删除。
  如同时配置单一 `COLLAB_OBJECT_KEK`，必须与映射中的当前版本一致。
- 生产开启附件或工作空间分享但缺少合法私有存储/对象 KEK，启动校验会拒绝；
  仅开启文本无需对象密钥。修改 `.env` 后按现有部署流程重建 API，单纯 restart
  不会更新容器环境。不要输出包含密钥的完整环境、Compose 展开结果或签名 URL。

当前自动化已覆盖合成配置和存储替身，不能替代真实桶、续传、双客户端与轮换演练。
这些出口完成前保持附件和工作空间分享关闭。此方案是服务端可授权解封的加密存储，
不是端到端加密，也无法撤回接收方已保存的明文副本。

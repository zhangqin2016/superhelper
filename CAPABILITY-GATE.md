...
| Lily platform skills are mistaken for OpenCode native `skill` commands, so `lily-*` capabilities appear "not found" | `test-opencode-config-builder.mjs` + `test-skill-catalog.mjs` + `test-capability-broker.mjs` + `test-runtime-pack-preflight.mjs` + `test-web-system-learning-intent.mjs` — OpenCode native `skill` is denied in runner permissions; AGENT.md/index/subagent prompts say `lily-*` are platform capability guides, not native skills; dependency/web-learning prompts route through Lily MCP/tools/scripts instead of `skill lily-*` |

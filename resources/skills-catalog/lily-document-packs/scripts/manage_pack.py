#!/usr/bin/env python3
"""Install / remove optional document capability packs from the agent.
...
contract with the main process (src/main/document-packs.js): pack files extract
to <userData>/document-packs/<id>/ and state lives in
<userData>/document-packs.json, so the main process picks installed packs up on
...
Usage:
  python manage_pack.py list
  python manage_pack.py status  <pack_id>
  python manage_pack.py install <pack_id>
  python manage_pack.py uninstall <pack_id>
...
# src/main/document-pack-specs.js. Available artifacts per platform are decided
...
def packs_root():
    return os.path.join(user_data_dir(), "document-packs")
...
def state_path():
    return os.path.join(user_data_dir(), "document-packs.json")
...
def resolve_artifact(pack_id):
    url = f"{api_base()}/api/document-packs/artifact?pack={pack_id}&platform={platform_key()}"

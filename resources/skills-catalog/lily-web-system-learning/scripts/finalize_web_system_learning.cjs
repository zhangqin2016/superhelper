...
function resolveMaybeRelative(file, baseDir) {
  if (!file) return "";
  return path.isAbsolute(file) ? file : path.resolve(baseDir, file);
}

function autoFrontendSourceFromScan(scan, scanPath) {
  const workDir = path.dirname(scanPath) || os.tmpdir();
  const harPath = resolveMaybeRelative(scan.harPath || "", workDir);
  if (!harPath || !fs.existsSync(harPath)) return { path: "", warning: "" };
  const outPath = path.join(workDir, "frontend-source-map.json");
  const script = path.join(__dirname, "frontend_source_intelligence.cjs");
  const argv = [
    script,
    "--har", harPath,
  ...
  for (const domain of (Array.isArray(scan.allowedDomains) ? scan.allowedDomains : []).filter(Boolean)) {
    argv.push("--allow-domain", domain);
  }
  const result = spawnSync(process.execPath, argv, { encoding: "utf8" });
  if (result.status !== 0) {
    return { path: "", warning: result.stderr || result.stdout || `frontend_source_intelligence exited ${result.status}` };
  }
  return { path: outPath, warning: "" };
}
...
function main() {
  ...
  let frontendSourcePath = args.frontendSource ? path.resolve(args.frontendSource) : "";
  let frontendSourceWarning = "";
  if (!frontendSourcePath) {
    const autoSource = autoFrontendSourceFromScan(scan, scanPath);
    frontendSourcePath = autoSource.path;
    frontendSourceWarning = autoSource.warning;
  }
  ...
  console.log(JSON.stringify({
    ...
    frontendSourceAutoGenerated: Boolean(frontendSourcePath && !args.frontendSource),
    frontendSourceWarning,

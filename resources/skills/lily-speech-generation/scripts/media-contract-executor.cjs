...
function normalizeParamValue(name, spec, input) {
  const hasInputValue = sourceValue(input, name) !== undefined && sourceValue(input, name) !== null && String(sourceValue(input, name)) !== "";
  let value = hasInputValue ? sourceValue(input, name) : spec.default;
...
function renderTemplateValue(value, params) {
  ...
  if (value && typeof value === "object") {
    const out = {};
    for (const [key, child] of Object.entries(value)) out[key] = renderTemplateValue(child, params);
    return out;
  }

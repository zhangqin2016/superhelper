...
- [IM Live Signals](2026-09-02-im-live-signals.md) — typing/已读双钩/表情回应三层打通。**关键**：typing 和已读回执的服务端早就做好了（realtime-gateway 已扇出、conversation.read 已带 recipientUserIds），缺的只是客户端；别再当服务端缺口重做。反应绝不能碰 message revision
- [Ask Path + Image Question Flow](2026-09-02-ask-path-and-image-flow.md) — 问答/图片问答全链路实测：空提示层每轮塞噪声还谎称有附件(759→187 字符)、裸 `daily/schedule/提醒我` 误判成定时任务白付一次模型调用、桥接描述当成亲眼所见。**重要纠正**：观测到的桥接轮次全是单图，并发化没解决那 30 秒——单图唯一杠杆是换带 vision 的主模型

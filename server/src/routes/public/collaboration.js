...
export function registerCollaborationRoutes(app, options = {}) { const { database = db, syncService = createCollaborationSyncService({ db: database }), ticketService = createCollaborationWsTicketService({ db: database }), friendService = createCollaborationFriendService({ repository: createKyselyFriendRepository(database) }) } = options; const defaults = options.messageService ? null : defaultMessageService(database); const messageService = options.messageService || defaults?.service || null; const authorizeMessage = options.authorizeMessage || defaults?.authorize || null; const authorizeReceipt = options.authorizeReceipt || authorizeMessage || createLockedMessageAuthorizer();
  ...
  post("/api/collaboration/v1/messages", commandBody.extend({ action: z.enum(["send", "edit", "revoke", "read", "history"]), conversationId: z.string().min(1), messageId: z.string().optional(), bodyText: z.string().max(65536).optional(), expectedRevision: z.number().int().positive().optional(), seq: z.number().int().min(0).optional(), beforeSeq: z.number().int().positive().optional(), limit: z.number().int().min(1).max(200).optional() }), async (request, reply) => {
    const input = commandBody.extend({ action: z.enum(["send", "edit", "revoke", "read", "history"]), conversationId: z.string().min(1), messageId: z.string().optional(), bodyText: z.string().max(65536).optional(), expectedRevision: z.number().int().positive().optional(), seq: z.number().int().min(0).optional(), beforeSeq: z.number().int().positive().optional(), limit: z.number().int().min(1).max(200).optional() }).parse(request.body); const account = await accountFor(request, reply, input, database); if (!account) return;

...
export function createConfiguredTaskService({database,config}){
  ...
  return createCollaborationTaskService({repository:createKyselyConversationRepository(database),crypto,packages:createTaskPackageBroker(),integrationLeasesEnabled:config.collaborationTaskGitEnabled===true&&config.collaborationSharedPublicationEnabled===true});

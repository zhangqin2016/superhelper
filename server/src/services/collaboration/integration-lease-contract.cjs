...
function leaseInput(value,action){
  ...
  if(action==='resolve'&&value?.afterTaskId!==undefined)allowed.push('afterTaskId');
  if(!['get','claim','renew','release','resolve'].includes(action)||!value||typeof value!=='object'||Array.isArray(value)
  ...
  if(action==='resolve'&&(value.expectedRevision!==0||value.afterTaskId!==undefined&&!id(value.afterTaskId)))fail();
